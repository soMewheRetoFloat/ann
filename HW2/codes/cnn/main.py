...
parser.add_argument('--struct_order', type=int, default=1)
...
if __name__ == '__main__':
	...
	arg_dict = vars(args)
	ms = arg_dict["struct_order"]
	wandb.init(
		project=f"cifar-10-cnn",
		config=arg_dict,
		name=f"extra cnn for pic turn"
		# name=f"cnn with lr: {arg_dict['learning_rate']} and dp: {arg_dict['drop_rate']} and batch: {arg_dict['batch_size']}"
	)
	...
	if args.is_train:
		...
		cnn_model = Model(drop_rate=args.drop_rate, conv_block_seed=ms)
		cnn_model.to(device)
		print(cnn_model)
