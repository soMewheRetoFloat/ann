...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq):
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            loss_value, acc_value = np.mean(loss_list), np.mean(acc_list)
            msg = '     Training iter %d, batch loss %.4f, batch acc %.4f' % (iter_counter, loss_value, acc_value)
            LOG_INFO(msg)

            wandb.log({
                "train_loss": loss_value,
                "train_acc": acc_value,
                "iteration_count": iter_counter
            })
...
def test_net(model, loss, inputs, labels, batch_size,):
    ...
    loss_value, acc_value = np.mean(loss_list), np.mean(acc_list)
    msg = '     Testing, total mean loss %.5f, total acc %.5f' % (loss_value, acc_value)
    LOG_INFO(msg)
