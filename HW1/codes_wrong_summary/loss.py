...
class KLDivLoss(object):
    ...
    def forward(self, input, target):
        ...
        softmax: y_i = exp(z_i) / sum(exp(z))

        loss: k_class(t_k(ln t_k - ln h_k)) / batch
        """
    ...
    def backward(self, input, target):
        # TODO START
        """
        grad[i , j] = ds_i/dx_j = softmax_j - t_i
        """
...
class HingeLoss(object):
    ...
    def forward(self, input, target):
        # TODO START
        """
        h_k = max(0, x_k - x_t^n + delta) if k != t_n
        """
        batch = input.shape[0]
    ...
    def backward(self, input, target):
        ...
        if have distance -> grad = 1 - target
        if no distance -> grad = 0
        """
        # Reference:
        # x_t^n
        respective_x_tn = np.sum(input * target, axis=1, keepdims=True)
...
class FocalLoss(object):
    ...
    def forward(self, input, target):
        # TODO START
        """
            suc_k = a * t_k + (1 - a) * (1 - t_k)
            focal = suc_k * (1 - softmax_h_k) ** gamma * cross_entropy
            -sum (focal)
        """
    ...
    def backward(self, input, target):
        ...
        """
        链式法则 一步一步算
        df / dx = df / d_softmax * d_softmax / dx

        df / d_softmax naive to get
        d_softmax / dx from jacobi matrix
